...
#' @export 
load_index = function(fasta = NULL, seq = NULL, seqname = 'myseq')
{

    if (!is.null(fasta))
        bwa = load_index_from_fasta(fasta)
    else
        bwa = load_index_from_sequence(seq, seqname)
    #tmp = data.table(names = c("khagay", "netha", "kevin"))
...
#' @export 
bwa = function(address, query, mc.cores = 1, tags = NULL)
{
    tmp = unlist(mclapply(query, run_bwa, address = address, mc.cores = mc.cores))
